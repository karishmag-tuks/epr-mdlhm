...
def sin_array(x):
    """Compute sine for array using math.sin - PARALLELIZED"""
    result = np.zeros_like(x)
    n = x.size
    for idx in prange(n):  # ← Flattened prange for better scheduling
        i = idx // x.shape[1]
        j = idx % x.shape[1]
...
@jit(nopython=True, fastmath=True, parallel=True)
def cos_array(x):
    """Compute cosine for array using math.cos - PARALLELIZED"""
    result = np.zeros_like(x)
    n = x.size
    for idx in prange(n):
        i = idx // x.shape[1]
...
# ============================================================================
# PRE-COMPUTED TWIDDLE FACTORS (Major optimization!)
# ============================================================================
...
def fft_1d_with_twiddles(x, twiddle_factors):
    """1D FFT using pre-computed twiddle factors (much faster!)"""
    n = x.shape[0]
    ...
    for s in range(1, num_stages + 1):
        m = 1 << s  # 2^s
        omega_m_idx_step = n // m  # Index step into twiddle array

        for k in range(0, n, m):
            omega_idx = 0
            for j in range(m // 2):
                # Lookup twiddle factor (fast!) instead of computing
...
def fft_2d_with_twiddles(x, twiddle_x, twiddle_y):
    ...
    for i in prange(M):  # ← prange for parallelization
        X[i, :] = fft_1d_with_twiddles(x[i, :], twiddle_x)

    # FFT on columns - PARALLELIZED!
    for j in prange(N):  # ← prange for parallelization
...
def compute_phase_A_vectorized(Py, Px, n, m, x_0, y_0, Delta_x_prime, Delta_y_prime,
    ...
    """Vectorized phase computation - MUCH faster!"""
    ...
    # Combine: phase_A[i,j] = k_over_L * (col_term[j] + row_term[i])
    for i in prange(Py):
        for j in range(Px):
...
def pad2x_rect(a):
    """Pad array to next power of 2 size"""
    Ny, Nx = a.shape
    Py = 1
    Px = 1
    while Py < 2 * Ny:
...
# ============================================================================
# KIRCHHOFF-HELMHOLTZ RECONSTRUCTION (ULTRA-OPTIMIZED)
# ============================================================================

class HologramReconstructor:
    """Ultra-optimized Kirchhoff-Helmholtz reconstruction with pre-computed constants"""

    def __init__(self, wavelength: float, L: float, pixel_size: float):
        self.wavelength = wavelength
        self.L = L * 1000
        self.pixel_size = pixel_size
        self.k = 2 * math.pi / wavelength

        # ⭐ PRE-COMPUTED CONSTANTS (Tier 1 optimization)
        self.k_over_L = self.k / self.L
        self.k_over_2L = self.k / (2 * self.L)
        self.L_squared = self.L * self.L
        self.two_pi = 2 * math.pi

        # ⭐ TWIDDLE FACTOR CACHE (Tier 2 optimization)
        self.twiddle_cache = {}

    def get_twiddle_factors(self, n):
        """Get or compute twiddle factors (cached)"""
        if n not in self.twiddle_cache:
            print(f"      Pre-computing twiddle factors for size {n}...")
            self.twiddle_cache[n] = precompute_twiddle_factors(n)
        return self.twiddle_cache[n]

    def coordinate_transform(self, X, Y):
        """Transform coordinates: X' = XL/R, Y' = YL/R"""
        R = np.sqrt(self.L_squared + X ** 2 + Y ** 2)
        L_over_R = self.L / R

        X_prime = X * L_over_R
        Y_prime = Y * L_over_R
        R_prime = self.L_squared / R

        return X_prime, Y_prime, R_prime

    def prepare_transformed_hologram(self, hologram, z):
        ...
        for i in range(M):
            for j in range(N):
                X[i, j] = coords_n[j] * self.pixel_size
                Y[i, j] = coords_m[i] * self.pixel_size

        X_prime, Y_prime, R_prime = self.coordinate_transform(X, Y)

        # Compute phase factors
        jacobian_factor = (self.L / R_prime) ** 4

        # Compute phase using pre-computed k_over_L
        phase_factor = np.zeros(R_prime.shape, dtype=np.complex64)
        phase_scale = self.k * z / self.L
        ...
        return I_prime, X_prime, Y_prime

    def compute_reconstruction_params(self, N: int, M: int,
                                      magnification_x: float = 1.0,
                                      magnification_y: float = 1.0):
        Delta_x_prime = self.pixel_size
        Delta_y_prime = self.pixel_size

        delta_x = (self.wavelength * self.L) / (N * Delta_x_prime) / magnification_x
        delta_y = (self.wavelength * self.L) / (M * Delta_y_prime) / magnification_y

        return delta_x, delta_y, Delta_x_prime, Delta_y_prime

    def reconstruct(self, hologram: np.ndarray, z: float,
                    magnification_x: float = 1.0, magnification_y: float = 1.0):
        """Main reconstruction with ultra-optimizations"""
        M, N = hologram.shape
        print(f"    Processing hologram {M}x{N}...")

        I_prime, X_prime, Y_prime = self.prepare_transformed_hologram(hologram, z)
        ...
        del X_prime, Y_prime
        print(f"    Prepared transformed hologram")

        I_pad, (sy, sx), (Ny, Nx) = pad2x_rect(I_prime)
        Py, Px = I_pad.shape
        del I_prime

        delta_x, delta_y, Delta_x_prime, Delta_y_prime = \
            self.compute_reconstruction_params(N, M, magnification_x, magnification_y)
        ...
        # ⭐ Use FFT with pre-computed twiddles (Tier 2 optimization)
        print(f"    Computing FFT2 ({Py}x{Px}) with pre-computed twiddles...")
        twiddle_x = self.get_twiddle_factors(Px)
        twiddle_y = self.get_twiddle_factors(Py)
        ...
        for i in range(Px):
            phase_R_x[i] = -(self.k * jx[i] ** 2 * delta_x * Delta_x_prime) / (2 * self.L)
        ...
        del R_x_input, phase_R_x

        phase_R_y = np.zeros(Py, dtype=np.float32)
        for i in range(Py):
            phase_R_y[i] = -(self.k * jy[i] ** 2 * delta_y * Delta_y_prime) / (2 * self.L)
        ...
        del convolution_result, phase_prefactor2, phase_prefactor3
...
def demonstrate_reconstruction():
    """Main reconstruction demonstration"""

    print("=" * 70)
    print("ULTRA-OPTIMIZED HOLOGRAM RECONSTRUCTION")
    ...
    reconstructor = HologramReconstructor(wavelength, L, pixel_size)
    ...
    for z in z_values:
        ...
        t0 = time.time()
        K = reconstructor.reconstruct(hologram, z, magnification_x=magnification_x,
    ...
    for i, (z, K) in enumerate(reconstructions):
        ...
        intensity_log = log10_array(intensity + 1e-10)
